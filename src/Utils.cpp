...
JType* cloneValue(JType* value) {
    ...
    if (typeid(*value) == typeid(JDouble)) {
        ...
        dupvalue =  runtime.heap->createPrimitive<JObject>();
        ...
        dupvalue =  runtime.heap->createPrimitive<JArray>();
...
JType* determineBasicType(const std::string& type) {
    if (IS_FIELD_INT(type) || IS_FIELD_BYTE(type) || IS_FIELD_CHAR(type) ||
        ...
        return runtime.heap->createPrimitive<JInt>();
    }
    if (IS_FIELD_DOUBLE(type)) {
        return runtime.heap->createPrimitive<JDouble>();
    }
    if (IS_FIELD_FLOAT(type)) {
        return runtime.heap->createPrimitive<JFloat>();
    }
    if (IS_FIELD_LONG(type)) {
        return runtime.heap->createPrimitive<JLong>();
    }
    return nullptr;
}
