...
void jjvm::methodCall(const std::string& name, const std::string& descriptor,
    ...
    auto&& codeAttr = std::static_pointer_cast<Code_attribute>(
        getAttr(method.attributes, AttributeTag::Code));
    if (codeAttr == nullptr) {
        ...
        for (size_t i = 0; i < descriptor.size() && descriptor[i] != ')';) {
            ...
            switch (static_cast<Field::FieldType>(descriptor[i])) {
                ...
                case Field::FieldType::ARRAY:
                    i = descriptor.find_first_not_of(
                        static_cast<char>(Field::FieldType::ARRAY), i);
