...
template <typename F>
std::vector<std::shared_ptr<ConstantPoolBase>>
read_constant_pools(std::basic_istream<F> &stream, uint16_t count) {
  ...
  for (uint8_t i = 1; i < count; i++) {
    ...
    case ConstantTag::CONSTANT_String: {
      ...
      auto utf8_infp =
          std::static_pointer_cast<CONSTANT_Utf8_info>(res[info->string_index]);
...
template <typename F>
std::vector<uint16_t> read_interfaces(std::basic_istream<F> &stream,
                                      uint16_t count) {
...
std::vector<uint8_t> read_code(std::basic_istream<F> &stream, uint32_t count) {
  ...
  for (i = 0; i < count; i++) {
    ...
    case Instruction::LOOKUPSWITCH:
      ...
      npairs =
          (res[i - 3] << 24) | (res[i - 2] << 16) | (res[i - 1] << 8) | res[i];
      if (npairs < 0) {
    ...
    case Instruction::TABLESWITCH:
      ...
      off = (res[i - 11] << 24) | (res[i - 10] << 16) | (res[i - 9] << 8) |
            res[i - 8];
      low = (res[i - 7] << 24) | (res[i - 6] << 16) | (res[i - 5] << 8) |
            res[i - 4];
      high =
          (res[i - 3] << 24) | (res[i - 2] << 16) | (res[i - 1] << 8) | res[i];
      ...
      for (j = low; j <= high; j++) {
        ...
        res[++i] = read_reverse<uint8_t>(stream);
        res[++i] = read_reverse<uint8_t>(stream);
        off = (res[i - 3] << 24) | (res[i - 2] << 16) | (res[i - 1] << 8) |
              res[i];
        if (base + off < 0 || base + off >= count) {
          throw std::logic_error("WTF!");
        }
...
template <typename F>
std::vector<Exception> read_exceptions(std::basic_istream<F> &stream,
                                       uint16_t count) {
...
template <typename F>
std::vector<std::shared_ptr<AttributeBase>>
read_attributes(std::basic_istream<F> &stream, uint16_t count,
                const std::vector<std::shared_ptr<ConstantPoolBase>> &cp) {
  ...
  for (uint16_t i = 0; i < count; i++) {
    ...
    case AttributeTag::InnerClasses: {
      ...
      for (uint16_t k = 0; k < number; ++k) {
        attr->classes[k].inner_class_info_index =
            read_reverse<uint16_t>(stream);
        attr->classes[k].outer_class_info_index =
            read_reverse<uint16_t>(stream);
        attr->classes[k].inner_name_index = read_reverse<uint16_t>(stream);
        attr->classes[k].inner_class_access_flags =
            read_reverse<uint16_t>(stream);
      }
    ...
    case AttributeTag::LocalVariableTable: {
      ...
      for (uint16_t k = 0; k < number; ++k) {
        ...
        attr->local_variable_table[k].name_index =
            read_reverse<uint16_t>(stream);
        attr->local_variable_table[k].descriptor_index =
            read_reverse<uint16_t>(stream);
...
template <typename F>
std::vector<Field>
read_fields(std::basic_istream<F> &stream, uint16_t count,
            const std::vector<std::shared_ptr<ConstantPoolBase>> &cp) {
...
template <typename F>
std::vector<Method>
read_methods(std::basic_istream<F> &stream, uint16_t count,
             const std::vector<std::shared_ptr<ConstantPoolBase>> &cp) {
...
ClassFile ClassFile::create(const std::string &file_name) {
  ...
  std::cout << "Super class: " << res.getClassName(res.super_class)
            << std::endl;
