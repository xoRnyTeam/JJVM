    ...
    for (decltype(codeLength) op = 0; op < codeLength; op++) {
        ...
        switch (code[op]) {
            ...
            case op_anewarray: {
                ...
                JArray *arrayref = runtime.heap->createObjectArray(
                    *symbolicRef.jc, count->val);
            ...
            default:
                cerr << "The JJVM can not recognize this opcode. Bytecode file "
                        "was be corrupted.";
                exit(EXIT_FAILURE);
        }
    }
...
bool Interpreter::checkInstanceof(const JavaClass *jc, u2 index,
    ...
    if (typeid(objectref) == typeid(JObject)) {
        ...
        if (tType == TYPE_CLASS) {
            ...
            auto *firstComponent =
                dynamic_cast<JObject *>(runtime.heap->getElement(
                    *dynamic_cast<JArray *>(objectref), 0));
