...
private:
  std::map<size_t, Type, std::less<>,
           HeapAllocator<std::pair<const size_t, Type>>>
      data;
};

template <typename Type> void Container<Type>::remove(size_t offset) {
  if (data.find(offset) != data.end()) {
    data.erase(offset);
  }
}

template <typename Type> size_t Container<Type>::place() {
  size_t lastOffset = 0;
  if (!data.empty()) {
    lastOffset = (--data.end())->first;
  }
  data.insert(std::make_pair(lastOffset + 1, Type{}));
  return lastOffset + 1;
}
...
using InternalPrimitive = JType *;
struct PrimitiveContainer : public Container<InternalPrimitive> {
  ~PrimitiveContainer() override {
    for (auto intPtr : getContainer()) {
      delete intPtr.second;
    }
  }
};
...
public:
  ...
  template <typename T, typename... Args> T *createPrimitive(Args &&...A) {
    T *type = new T(std::forward<Args>(A)...);
    auto offset = primitiveContainer.place();
    primitiveContainer.find(offset) = type;
    return type;
  }
  ...
  void putFieldByOffset(const JObject &object, size_t fieldOffset,
                        JType *value) {
    // assert(objectContainer.find(object.offset)[fieldOffset] != nullptr &&
    // "!!!!!!!!!!!!!!!!!!!");
  ...
  auto getElements(JArray *array) { return arrayContainer.find(array->offset); }

  void removeArray(size_t offset) { arrayContainer.remove(offset); }
  void removeObject(size_t offset) { objectContainer.remove(offset); }
