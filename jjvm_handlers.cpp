...
void jjvm::opdastore() {
  ...
  va = m_frames.top().stackTop();
  m_frames.top().stackPop();
  m_heap.store(va.heapID, vi.i, vv.d);
}

void jjvm::oppop() { m_frames.top().stackPop(); }
...
void jjvm::opiinc() {
  ...
  int8_t c =
      static_cast<int8_t>(m_frames.top().code->code[m_frames.top().pc++]);
  v = m_frames.top().localLoad(i);
  v.i += c;
  m_frames.top().localStore(i, v);
}
...
void jjvm::opreturn() { m_frames.top().pc = -1; }
...
void jjvm::opnewarray() {
  TypeCode type =
      static_cast<TypeCode>(m_frames.top().code->code[m_frames.top().pc++]);
  Value v = m_frames.top().stackTop();
  m_frames.top().stackPop();
...
void jjvm::intiHandlers() {
  ...
  // m_handlers[INVOKEVIRTUAL]   = std::bind(&jjvm::opinvokevirtual,
  // this);//TODO
